...
export function GoogleSignInButton({ role }: { role?: 'musician' | 'organizer' }) {
  ...
  const signIn = async () => {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        redirectTo: `${window.location.origin}/auth/callback?next=${role === 'musician' ? '/dashboard' : '/'}`,
        queryParams: role ? { prompt: 'select_account' } : {},
      },
    })
    if (error) {
